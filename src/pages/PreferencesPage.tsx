...
export function PreferencesPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gray-900 rounded-lg p-4">
            ...
            <p className="text-lg font-medium text-white">{profile?.full_name || 'Not set'}</p>
          </div>
          <div className="bg-gray-900 rounded-lg p-4">
            <p className="text-gray-400 text-sm">Client Type</p>
            <p className="text-lg font-medium text-white capitalize">{profile?.client_type || 'Not set'}</p>
          </div>
          <div className="bg-gray-900 rounded-lg p-4">
            <p className="text-gray-400 text-sm">Market Location</p>
            <p className="text-lg font-medium text-white">{profile?.market_location || 'Not set'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-6">
        <h2 className="text-xl font-semibold text-white mb-4">Learned Preferences</h2>
            ...
            <div className="flex flex-wrap gap-2">
              {preferences?.preferred_makes && preferences.preferred_makes.length > 0 ? (
                preferences.preferred_makes.map((make: string) => (
                  <span key={make} className="bg-blue-500/20 text-blue-400 px-3 py-1 rounded-full text-sm">{make}</span>
                ))
              ) : (
          ...
          <div>
            ...
            <p className="text-gray-300">
              {preferences?.preferred_price_min || preferences?.preferred_price_max ? (
                <span>${(preferences?.preferred_price_min || 0).toLocaleString()} - ${(preferences?.preferred_price_max || 0).toLocaleString()}</span>
              ) : (
            ...
            <div className="flex flex-wrap gap-2">
              {preferences?.preferred_locations && preferences.preferred_locations.length > 0 ? (
                preferences.preferred_locations.map((loc: string) => (
                  <span key={loc} className="bg-purple-500/20 text-purple-400 px-3 py-1 rounded-full text-sm">{loc}</span>
                ))
              ) : (
